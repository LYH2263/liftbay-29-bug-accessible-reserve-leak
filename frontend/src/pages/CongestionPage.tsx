...
export default function CongestionPage() {
  ...
  const total = rows.reduce((n, r) => n + r.passengers, 0);
  return (<>
    <h2>拥堵 <span className="list-total mono">候梯合计 {total} 人</span></h2>
    ...
    <tbody>{rows.map(r => <tr key={r.floor}><td className="mono">{r.floor}F</td><td>{r.passengers}</td>
      ...
      {rows.length > 0 && <tr className="list-total-row"><td>合计</td><td>{total}</td><td></td></tr>}
